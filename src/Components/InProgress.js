...
const progressList = (setMainState,openModal,props) => {
        ...
        return<div  style={{position:'relative',textAlign:'left',height:'55px',lineHeight:'55px',overflow:'hidden'}}>
            ...
            <div className='inprogress'>
                ...
                <div onClick={()=>review(props,props.userID,setMainState,openModal,Review)} style={{color:'white',fontSize:'0.8em',textAlign:'center',position:'absolute',right:'0',backgroundColor:'red',width:'50px',height:'100%'}}>Done</div>
...
const makeInList = (setMainState,openModal,props) => {
    let result = [];
    ...
    for (let i in props){
        console.log(props[i])
        if(i==props.length-1)result.push(progressList(setMainState,openModal,props[i]))
        else result.push(progressList(setMainState,openModal,props[i]))
    }
...
const InProgress = ({setMainState,openModal,...props}) => (
    <div>
    ...
    <div style={{position: 'relative',top:'300px'}}>
        ...
        {makeInList(setMainState,openModal,props.inprogress)}
    </div>
    }</div>
)
